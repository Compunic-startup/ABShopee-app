...
// //     cart?.items?.some(i => i.itemSnapshot?.itemType === 'physical')
...
// //         `${BASE_URL}/customer/business/${businessId}/cart`,
...
// //       const res = await fetch(
// //         `${BASE_URL}/customer/business/${businessId}/items/${item.itemId}/cart/add`,
// //         {
// //           method: 'POST',
// //           headers: {
...
// //       const businessId = await AsyncStorage.getItem('businessId')
// //       const res = await fetch(
// //         `${BASE_URL}/customer/business/${businessId}/items/cart/${cartItemId}`,
// //         {
// //           method: 'DELETE',
// //           headers: {
// //             Authorization: `Bearer ${token}`,
...
// //   const buildOrderPayload = ({ cartId, cartItems, paymentMethod, address, email }) => {
// //     const hasDigital = cartItems.some(
...
// //             name: address.name,
// //             phone: address.phone,
// //             email: hasDigitalItem ? digitalEmail : address.email,
// //           },
// //         },
// //       ]
// //     }
...
// //         if (!digitalEmail) {
// //           ToastAndroid.show(
// //             'Enter email for digital delivery',
// //             ToastAndroid.SHORT
// //           )
// //           return
// //         }

// //         if (!digitalEmail.includes('@')) {
// //           ToastAndroid.show(
...
// //       if (
// //         paymentMethod === 'COD' &&
// //         cartItems.some(i => i.itemSnapshot?.itemType === 'digital')
// //       ) {
// //         ToastAndroid.show(
// //           'COD not allowed for digital items',
// //           ToastAndroid.SHORT
// //         )
// //         return
// //       }
...
// //       const res = await fetch(
// //         `${BASE_URL}/customer/business/${businessId}/order/place`,
// //         {
// //           method: 'POST',
// //           headers: {
// //             'Content-Type': 'application/json',
...
// //             {cart?.items?.map((item) => {
// //               const base =
// //                 item.discountPricing?.baseItemTotal ??
// //                 item.pricing?.itemTotal ??
// //                 0

// //               const final =
// //                 item.discountPricing?.finalItemTotal ??
// //                 item.pricing?.itemTotal ??
// //                 0

// //               const discount =
// //                 item.discountPricing?.discountTotal ?? 0

// //               const unitPrice =
// //                 item.discountPricing?.finalUnitPrice ??
// //                 final / item.quantity
...
// //                   <Text style={styles.offerText}>Use Loyalty Points</Text>
// //                 </View>
// //                 <Icon name="chevron-right" size={24} color="#999" />
// //               </TouchableOpacity>
// //             </View>)}
...
// //                 Enter email to receive digital products
// //               </Text>
...
// //                 ]}>
// //                   {paymentMethod === 'COD' && !hasDigitalItem && (
...
// //                   name="cash"
// //                   size={20}
// //                   color={hasDigitalItem ? '#ccc' : '#4CAF50'}
// //                 />
// //                 <View>
// //                   <Text style={[
// //                     styles.paymentTitle,
// //                     hasDigitalItem && styles.paymentDisabled,
// //                   ]}>
// //                     Cash on Delivery
// //                   </Text>
// //                   {hasDigitalItem && (
// //                     <Text style={styles.paymentSubtitle}>
// //                       Not available for digital items
// //                     </Text>
// //                   )}
// //                 </View>
// //               </View>
...
// //               Digital orders cannot be returned. Physical orders can be returned within 7 days.
// //               Refund will be processed in 2-3 working days.
// //             </Text>
// //           </View>
...
//     i => i.itemSnapshot?.itemType === 'physical'
//   )
...
//       const res = await fetch(`${BASE_URL}/customer/business/${bId}/cart`, {
//         headers: { Authorization: `Bearer ${token}` },
//       })
//       const json = await res.json()
//       console.log('CART DATA', json)
...
//       (acc, item) => {
//         const base =
//           item.discountPricing?.baseItemTotal ?? item.pricing?.itemTotal ?? 0
//         const final =
//           item.discountPricing?.finalItemTotal ?? item.pricing?.itemTotal ?? 0
//         const discount = item.discountPricing?.discountTotal ?? 0
//         const tax = item.taxBreakdown ?? {}
...
//   // ─── Remove single item ───────────────────────────────────────────────────
//   // BUG FIX: removed the 'ALL' case from this function – it had no valid API
//   const removeItem = async cartItemId => {
//     try {
//       const token = await AsyncStorage.getItem('userToken')
//       const bId = await AsyncStorage.getItem('businessId')
...
//   // BUG FIX: dedicated clear-cart function using the correct endpoint
...
//   // ─── Build order payload ──────────────────────────────────────────────────
//   // BUG FIX: was referencing outer `hasDigitalItem` variable instead of the
//   //          locally computed `hasDigital` derived from the passed `cartItems`.
//   const buildOrderPayload = ({ cartId, cartItems, paymentMethod, address, email }) => {
...
//             name: address.name,
//             phone: address.phone,
//             // BUG FIX: was using `hasDigitalItem` (outer scope) which is
//             // unreliable inside this helper; now uses local `hasDigital`
...
//   // ─── Place order ──────────────────────────────────────────────────────────
//   // BUG FIX (multiple):
//   //   1. `businessId` parameter was immediately shadowed by AsyncStorage call.
//   //   2. Razorpay email used `address.email` even for digital orders.
//   //   3. `fetchCart` no longer needs to be passed as a param – it's in scope.
...
//       const token = await AsyncStorage.getItem('userToken')
//       // BUG FIX: use state-level businessId; no shadowing
...
//       if (json.paymentMethod === 'RAZORPAY') {
//         // BUG FIX: use digitalEmail for digital orders, address.email for physical
//         openRazorpay({
//           razorpayOrder: json.razorpay,
//           orderId: json.orderId,
//           navigation,
...
//               const discount = item.discountPricing?.discountTotal ?? 0
//               const unitPrice =
//                 item.discountPricing?.finalUnitPrice ?? final / item.quantity
...
//           {/* Clear Cart Button */}
//           {/* BUG FIX: was calling removeItem('ALL') — now calls clearCart() */}
//           {cart?.items?.length > 1 && (
...
//                     color="#4CAF50"
//                   />
//                   <Text style={styles.offerText}>Use Dealer Code</Text>
//                 </View>
//                 <Icon name="chevron-right" size={24} color="#999" />
//               </TouchableOpacity>
//             </View>
...
//       {/* Bottom Bar */}
//       {/* BUG FIX: removed stray `1` text node after </TouchableOpacity> */}
//       <View style={styles.bottomBar}>
//         <View style={styles.bottomLeft}>
//           <Text style={styles.bottomTotal}>₹{grandTotal.toFixed(2)}</Text>
//           <Text style={styles.bottomSubtext}>
